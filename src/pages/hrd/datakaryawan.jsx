...
const DataKaryawanHRD = () => {
  ...
  const [formData, setFormData] = useState({
    nama: "", nik: "", password: "", role: "karyawan", cabang_id: "", jabatan: "", divisi: "", no_telp: ""
  });
    ...
    <div className="hrd-container">
      ...
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
        ...
        <nav className="menu-nav">
          ...
          <div className="menu-item has-arrow" onClick={() => handleNav('/hrd/kehadiran')}><div className="menu-left"><img src={iconKehadiran} alt="" className="menu-icon-main" /><span className="menu-text-main">Kehadiran</span></div><img src={iconBawah} alt="down" className="arrow-icon-main" /></div>
        ...
        <div className="dk-action-row">
          <div className="dk-action-group">
            <div className="filter-wrapper">
              <button className="btn-dk-filter" onClick={() => setShowFilter(!showFilter)}>
                {selectedCabang} <img src={iconBawah} alt="v" style={{ width: '10px', filter: 'brightness(0) invert(1)', transform: showFilter ? 'rotate(180deg)' : 'none', transition: '0.2s' }} />
              </button>
              {showFilter && (
                <div className="filter-dropdown">
                  <div className="dropdown-item" onClick={() => {setSelectedCabang("Semua Cabang"); setShowFilter(false)}}>Semua Cabang</div>
                  {cabangList.map(c => <div key={c.id} className="dropdown-item" onClick={() => {setSelectedCabang(c.nama); setShowFilter(false)}}>{c.nama}</div>)}
                </div>
              )}
            </div>
          ...
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleFormSubmit}>
              <div className="modal-grid-2">
                ...
                <div className="form-group"><label>Password Login</label><input type="text" className="input-edit" required onChange={(e) => setFormData({...formData, password: e.target.value})} /></div>
